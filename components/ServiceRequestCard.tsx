...
import { Wrench, Check, X, Loader2, Pencil, User } from "lucide-react";
						...
						<div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
							{request.sparePartsNeeded.map((p, idx) => (
								<div key={idx} className="rounded border p-2 flex items-center justify-between">
									...
									{p.cost ? (
										<div className="text-xs text-muted-foreground">{(Number(p.cost) * Number(p.quantity || 0)).toFixed(2)}</div>
									) : null}
								</div>
							))}
						</div>
					...
					<div className="space-y-1">
						...
						{request.approvalStatus === ServiceRequestApprovalStatus.PENDING && canApprove ? (
							<div className="flex gap-2">
								<Button size="sm" className="gap-1" disabled={isUpdatingApproval} onClick={() => onApprove?.(request.id)}>
									{isUpdatingApproval ? <Loader2 className="h-4 w-4 animate-spin" /> : <Check className="h-4 w-4" />} {t("serviceRequest.statuses.approved")}
								</Button>
								<Button size="sm" variant="destructive" className="gap-1" disabled={isUpdatingApproval} onClick={() => onReject?.(request.id)}>
									{isUpdatingApproval ? <Loader2 className="h-4 w-4 animate-spin" /> : <X className="h-4 w-4" />} {t("serviceRequest.statuses.rejected")}
								</Button>
							</div>
						) : (
					...
					<div className="space-y-1">
						...
						{request.workStatus === ServiceRequestWorkStatus.PENDING && request.approvalStatus === ServiceRequestApprovalStatus.APPROVED ? (
							<div className="flex gap-2">
								...
								<Button size="sm" variant="outline" className="gap-1" disabled={isUpdatingWork} onClick={() => onCancel?.(request.id)}>
									{isUpdatingWork ? <Loader2 className="h-4 w-4 animate-spin" /> : <X className="h-4 w-4" />} {t("serviceRequest.statuses.cancelled")}
								</Button>
							</div>
						) : (
				...
				<div className="flex gap-2">
					{canEditDetails && (
						<ServiceRequestDialog
							...
							existing={request}
							trigger={<Button size="sm" variant="outline"><Pencil className="h-4 w-4 mr-1 rtl:mr-0 rtl:ml-1" />{t("equipment.edit")}</Button>}
						/>
					)}
				</div>
			</CardContent>
