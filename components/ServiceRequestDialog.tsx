...
const ServiceRequestDialog = ({ equipmentId, equipmentName, trigger, existing, onCreated }: ServiceRequestDialogProps) => {
  const { t } = useLanguage();
      ...
      <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-3xl">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            {existing ? t("serviceRequest.editTitle") : t("serviceRequest.createTitle")} {equipmentName ? <Badge variant="secondary" className="ml-2">{equipmentName}</Badge> : null}
          </DialogTitle>
          <DialogDescription>
          ...
          <div className="space-y-4">
            ...
            <div className="flex flex-col gap-2">
              ...
              <div className="flex flex-col gap-2">
                ...
                <Select value={form.assignedTechnicianId} onValueChange={(v) => setForm((s) => ({ ...s, assignedTechnicianId: v }))} disabled={techLoading}>
                  <SelectTrigger>
        ...
        <DialogFooter>
          {step > 0 ? (
            <Button variant="outline" onClick={() => setStep((s) => ((s - 1) as 0 | 1 | 2))} disabled={loading || isBlockingLoading}>
              {t("form.back")}
            </Button>
          ) : (
          ...
          {step < 2 ? (
            <Button onClick={() => setStep((s) => ((s + 1) as 0 | 1 | 2))} disabled={(step === 0 && !isValid) || loading || isBlockingLoading}>
              {t("form.next")}
            </Button>
          ) : (
