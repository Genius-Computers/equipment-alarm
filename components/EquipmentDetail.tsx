...
const EquipmentDetail = ({ equipmentId, canonicalizeToTag = false }: EquipmentDetailProps) => {
  ...
  useEffect(() => {
    ...
    void loadEquipmentById(equipmentId);
  }, [equipmentId, loadEquipmentById]);

  useEffect(() => {
    if (!equipmentId) return;
    void loadForEquipment(equipmentId, { page, pageSize, scope });
  }, [equipmentId, page, pageSize, scope, loadForEquipment]);

  // Canonicalize to tag-based URL when on UUID route
  useEffect(() => {
    if (!canonicalizeToTag) return;
    if (!currentEquipment?.partNumber) return;
    if (pathname?.startsWith("/equipments/tag/")) return;
    const tag = currentEquipment.partNumber;
    if (typeof tag === "string" && tag.trim().length > 0) {
      router.replace(`/equipments/tag/${encodeURIComponent(tag)}`);
    }
  }, [canonicalizeToTag, currentEquipment?.partNumber, pathname, router]);
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  {requests.map((req) => (
                    <ServiceRequestCard
                      ...
                      onEdited={() => loadForEquipment(equipmentId, { page, pageSize, scope })}
                    />
                  ))}
                </div>
              )}
